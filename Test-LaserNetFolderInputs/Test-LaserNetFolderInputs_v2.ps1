...
Write-Verbose -Message "CHECKING IF ONLY destination0 HAS BEEN SET."
IF ($destination0 -and !$destination1 -and !$destination2 -and !$destination3 -and !$destination4)
{
	Get-ChildItem $source | ForEach-Object {
		IF ($WhatIf -eq $True)
		{
			Write-Verbose -Message "PERFORM MOVE PROCECSS TO destination0 WITH -WhatIf PARAMETER SET."
			Move-Item $_.FullName -Destination $destination0 -Force -WhatIf
		}
		ELSE
		{
			Write-Verbose -Message "PERFORM MOVE PROCESS TO destination0."
			Move-Item $_.FullName -Destination $destination0 -Force
		}
		...
		IF ($count -eq $breakpoint)
		{
			...
			While ([bool](Get-ChildItem $destination0))
			{
...
Write-Verbose -Message "CHECKING IF ONLY destination0 AND destination1 HAVE BEEN SET."
IF ($destination0 -and $destination1 -and !$destination2 -and !$destination3 -and !$destination4)
{
	Write-Verbose -Message "CHECK WHETHER OR NOT destination1 ENDS WITH '\'. IF IT DOES NOT, THE CHARACTER IS ADDED."
	IF ($destination1[-1] -ne "\") {$destination1 = $destination1 + "\"}
	Write-Verbose -Message "CHECKING IF destination1 IS A VALID DIRECTORY.  IF NOT, IT IS CREATED."
	...
	Get-ChildItem $source | ForEach-Object {
		...
		ELSE
		{
			Write-Verbose -Message "PERFORM MOVE PROCESS TO destination."
...
Write-Verbose -Message "CHECKING IF ONLY $destination0, destination1 AND destination2 HAVE BEEN SET."
IF ($destination0 -and $destination1 -and $destination2 -and !$destination3 -and !$destination4)
{
	Write-Verbose -Message "CHECK WHETHER OR NOT destination1 AND destination2 END WITH '\'. IF THEY DO NOT, THE CHARACTER IS ADDED."
	IF ($destination1[-1] -ne "\") {$destination1 = $destination1 + "\"}
	IF ($destination2[-1] -ne "\") {$destination2 = $destination2 + "\"}
	Write-Verbose -Message "CHECKING IF destination1 AND destination2 ARE VALID DIRECTORIES.  IF NOT, THEY ARE CREATED."
	IF (!(Test-Path $destination1)) {New-Item $destination1 -ItemType "Directory" -Force}
...
Write-Verbose -Message "CHECKING IF ONLY destination0, destination1, destination2 AND destination3 HAVE BEEN SET."
IF ($destination0 -and $destination1 -and $destination2 -and $destination3 -and !$destination4)
{
	Write-Verbose -Message "CHECK WHETHER OR NOT destination1, destination2 AND destination3 END WITH '\'. IF THEY DO NOT, THE CHARACTER IS ADDED."
	IF ($destination1[-1] -ne "\") {$destination1 = $destination1 + "\"}
	IF ($destination2[-1] -ne "\") {$destination2 = $destination2 + "\"}
	IF ($destination3[-1] -ne "\") {$destination3 = $destination3 + "\"}
	Write-Verbose -Message "CHECKING IF destination1, destination2 AND destination3 ARE VALID DIRECTORIES.  IF NOT, THEY ARE CREATED."
	IF (!(Test-Path $destination1)) {New-Item $destination1 -ItemType "Directory" -Force}
	IF (!(Test-Path $destination2)) {New-Item $destination2 -ItemType "Directory" -Force}
...
Write-Verbose -Message "CHECKING IF ALL DESTINATION VARIABLES HAVE BEEN SET."
IF ($destination0 -and $destination1 -and $destination2 -and $destination3 -and $destination4)
{
