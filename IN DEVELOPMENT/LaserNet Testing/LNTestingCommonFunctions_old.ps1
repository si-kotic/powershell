...
Function lnTest-Conf {
	...
	$global:inputFolder0 = "$root\$testID\File_Input0\"
	$global:inputFolder1 = "$root\$testID\File_Input1\"
	$global:inputFolder2 = "$root\$testID\File_Input2\"
	$global:inputFolder3 = "$root\$testID\File_Input3\"
	$global:inputFolder4 = "$root\$testID\File_Input4\"
	$global:finalDest = "$root\$testID\File_Output\"
	$global:processingSource = "$root\$testID\Processing\"
	$global:source = "$root\$testID\Source\"
	$global:sample = "$root\$testID\Sample\"
	$global:inPrinter = Get-WmiObject -Class Win32_Printer | Where {$_.Name -eq "LaserNetInput"} #Probably needs changing!
...
$numJobs
	...
	$global:tempPath = "$root\$testID\TempInData"
	$outData = "$root\$testID\TempInData\SampleInputData.txt"
	New-Item -Path $tempPath -Name SampleInputData.txt -ItemType File -Force
	$formFeed = Get-Content 'C:\LaserNet Testing\6.7.0\Full Testing\Resources\LOAD\CommonComponents\FormFeed.txt'
...
$numJobs
	...
	$global:tempPath = "$root\$testID\TempInData"
	New-Item -Path "$root\$testID" -Name TempInData -ItemType Directory -Force
	For ($i=0;$i -lt $numJobs;$i++)
		{
			Copy-Item -Path (Get-ChildItem $sample).FullName -Destination ($tempPath + "\" + $testID + "_" + $i + (Get-ChildItem $sample).Extension) -Force
		}
}
...
Function Export-LNTestingTable {
	...
	$table | Export-Csv "$processingSource\test.csv"
	$csv = Get-Content "$processingSource\test.csv"
	$csv[1..$csv.Count] | Set-Content "$processingSource\test.csv"
}