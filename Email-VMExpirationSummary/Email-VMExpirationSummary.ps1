...
Start-Transcript -Path ((Split-Path $MyInvocation.MyCommand.Path) + "\Email-VMExpirationSummary.log")
...
$table | Where {$_.resourcePool -ne "Resources"} | Group-Object resourcePool | ForEach-Object {
	...
	$html += $_.Group | Select vmName,powerStatus,purpose,expiryDate,vmOwner,vmToolsStatus,notes | ConvertTo-Html -Fragment | Foreach-Object {
...
ConvertTo-Html -InputObject $null <#-CssUri $css#> -Head $head -Title "Virtual Machine Inventory Summary" -PreContent ("<H4>" + $msgBody + "</H4>") -PostContent $html | Out-File 'C:\Scripts\vmOwnersAndExpiry.html'
Send-MailMessage -To $adminEmail -From $msgFrom -SmtpServer $smtpServer -Subject $msgSubject -BodyAsHTML (ConvertTo-Html -InputObject $null <#-CssUri $css#> -Head $head -Title "Virtual Machine Inventory Summary" -PreContent ("<H4>" + $msgBody + "</H4>") -PostContent $html | out-string) -Attachments 'C:\Scripts\vmOwnersAndExpiry.html'
